...
public class GHClockView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 60;++i)
        {
            ...
            if (i % 15 == 0) drawHourText(dr-40,(int) (i/5),canvas,paint);
        }
    ...
    {
        double degree = (2*Math.PI)*hour/12.0;
        return (int)(r*Math.sin(degree)+half_w());
    }

    private int getMinuteX(int r,int minute)
    {
        double degree = (2*Math.PI)*minute/60.0;
        return (int)(r*Math.sin(degree)+half_w());
    }
